...
def generate(message, account_id, temperature, model):
                                full_response = ""
                                for chunk in run_inference_stream(message, account_id, temperature, model, stream=True):
                                    full_response += chunk
                                    yield chunk
...
# Inference service
# The backend no longer talks directly to Ollama.
# Change this to the URL where Worker-inference is running.
INFERENCE_URL = "http://localhost:11434/generate"


def run_inference_stream(prompt, account_id, temperature=0.2, model="LlaMa3.2", stream=True):
    """Send an inference request to Worker-inference.

    Expected endpoint: POST /generate
    Expected JSON: prompt, model, temperature, stream
    Expected streaming format: JSON lines containing a "response" field.

    Account data and conversation history remain on this backend.
    The account_id is not sent to the inference service.
    """
    account_dir = Path(ATLAS_DIR / "data/accounts") / account_id

    with open(account_dir / "account.json", "r", encoding="utf-8") as f:
        account_data = json.load(f)
        name = account_data["Name"]

    context = ""
    with open(account_dir / "history" / "conversations.json", "r", encoding="utf-8") as f:
        conversation_data = json.load(f)

    for chat in conversation_data.get("Chats", []):
...
"""
    ...
    if response.status_code != 200:
        print("Inference worker status:", response.status_code)
        print("Inference worker response:", response.text)
